...
class TyphoonModel:
    def __init__(self):
        self.player_pos = [SCREEN_WIDTH // 2, SCREEN_HEIGHT - 80]  # 將初始位置往上移動並向左移動10
        self.umbrella_direction = 'RIGHT'
        self.wind_direction = 'LEFT'
        self.last_wind_change = pygame.time.get_ticks()
        self.obstacles = []
        self.generate_obstacle_interval = 1000  # 每隔0.5秒生成障礙物
        ...
        self.move_speed = 2  # 調整角色的移動速度，使其更慢
        self.lane_start_x = (SCREEN_WIDTH - (self.lane_width * 5)) // 2 - 20  # 水平置中並向左移動10
    ...
    def update(self):
        ...
        if current_time - self.last_obstacle_time > self.generate_obstacle_interval:
            # 更新障礙物數量和位置
            num_obstacles = random.randint(1, 3)  # 隨機決定本次生成的障礙物數量
            new_obstacles = []
